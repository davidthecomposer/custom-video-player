...
class VideoPlayer {
			...
			this.video.onprogress = () => {
				...
				if (duration > 0) {
					for (let i = 0; i < this.video.buffered.length; i++) {
						if (
							this.video.buffered.start(this.video.buffered.length - 1 - i) <
							this.video.currentTime
						) {
							bufferedProgress.value =
								(this.video.buffered.end(this.video.buffered.length - 1 - i) /
									duration) *
								100;
							break;
						}
	...
	getDuration() {
		let time =
			Math.floor(this.video.duration.toFixed(0)) -
			Math.floor(this.video.currentTime.toFixed(0));
	...
	getTime() {
		this.currentPosition.innerHTML =
			Math.floor(this.video.currentTime.toFixed(0) / 60) +
			":" +
			(this.video.currentTime.toFixed(0) < 10
				? `0${this.video.currentTime.toFixed(0)}`
				: this.video.currentTime.toFixed(0) % 60 < 10
				? `0${this.video.currentTime.toFixed(0) % 60}`
				: this.video.currentTime.toFixed(0) % 60
				? this.video.currentTime.toFixed(0) % 60
				: "00");
		this.getDuration();
